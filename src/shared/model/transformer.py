...
class DecoderOnlyTransformer(L.LightningModule):
    ...
    def compute_chunked_loss(
        ...
        flat_hidden_states = hidden_states.reshape(-1, hidden_states.size(dim=-1))
        token_count = flat_hidden_states.size(dim=0)
        ...
        loss_chunks = [
            self.loss(
                self.fc_layer(
                    flat_hidden_states[chunk_start : chunk_start + self.loss_chunk_size, :]
                ),
                flat_labels[chunk_start : chunk_start + self.loss_chunk_size],
            )
...
def build_packed_attention_mask(segment_ids: torch.Tensor) -> torch.Tensor:
    ...
    # same packed segment while keeping padding labels ignored.
    # ---------------------------------------------------------
